...
import { getLocationDetail, updateLocation, deleteLocation } from "@/lib/db/location";
...
export const GET = withAuth(async (req, user, { params }) => {
  ...
  try {
    ...
    return NextResponse.json({ code: "INTERNAL_ERROR", message: "Failed to fetch location details" }, { status: 500 });
  }
});
...
export const PATCH = withAuth(async (req, user, { params }) => {
  ...
  try {
    ...
    return NextResponse.json({ code: "INTERNAL_ERROR", message: "Failed to update location" }, { status: 500 });
  }
});
...
export const DELETE = withAuth(async (req, user, { params }) => {
  ...
  try {
    ...
    return NextResponse.json({ code: "INTERNAL_ERROR", message: "Failed to delete location" }, { status: 500 });
  }
});
