...
export const GET = withAuth(async (req, payload, context: { params: Promise<{ slug: string, id: string }> }) => {
  try {
    ...
    return NextResponse.json({ code: "INTERNAL_ERROR", message: "Failed to fetch QR code details" }, { status: 500 });
  }
});
 ...
 */
export const PATCH = withAuth(async (req, payload, context: { params: Promise<{ slug: string, id: string }> }) => {
  try {
    const { slug, id } = await context.params;
    ...
    return NextResponse.json({ code: "INTERNAL_ERROR", message: "Failed to update QR code" }, { status: 500 });
  }
});
 ...
 */
export const DELETE = withAuth(async (req, payload, context: { params: Promise<{ slug: string, id: string }> }) => {
  try {
    const { slug, id } = await context.params;
    ...
    return NextResponse.json({ code: "INTERNAL_ERROR", message: "Failed to delete QR code" }, { status: 500 });
  }
});
