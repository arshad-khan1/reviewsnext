  ...
  try {
    ...
    if (planCheck.error === "PLAN_REQUIRED") {
      return NextResponse.json(
        { code: "PLAN_REQUIRED", message: "Per-QR branding requires PRO plan" },
        { status: 403 },
      );
    }
    ...
    return NextResponse.json(
      { code: "INTERNAL_ERROR", message: "Failed to fetch QR branding" },
      { status: 500 },
    );
  }
});
...
export const PUT = withAuth(async (req, user, { params }) => {
  ...
  try {
    ...
    return NextResponse.json(
      { code: "INTERNAL_ERROR", message: "Failed to update QR branding" },
      { status: 500 },
    );
  }
});
...
export const DELETE = withAuth(async (req, user, { params }) => {
  ...
  try {
    ...
    return NextResponse.json(
      { code: "INTERNAL_ERROR", message: "Failed to reset QR branding" },
      { status: 500 },
    );
  }
});
