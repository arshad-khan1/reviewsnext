...
import { findBusinessBySlug, updateBusiness, deleteBusiness, isBusinessOwner } from "@/lib/db/business";
...
export const GET = withAuth(async (req, payload, context: { params: Promise<{ slug: string }> }) => {
  try {
    ...
    return NextResponse.json({ code: "INTERNAL_ERROR", message: "Failed to fetch business" }, { status: 500 });
  }
});
...
const patchBusinessSchema = z.object({
  ...
  defaultCommentStyle: z.enum(["PROFESSIONAL_POLITE", "FRIENDLY_CASUAL", "CONCISE_DIRECT", "ENTHUSIASTIC_WARM"]).optional(),
});
 ...
 */
export const PATCH = withAuth(async (req, payload, context: { params: Promise<{ slug: string }> }) => {
  try {
    const { slug } = await context.params;

    const owner = await isBusinessOwner(payload.sub, slug);
    if (!owner) {
      // Check if business exists to determine 403 vs 404
    ...
    const result = patchBusinessSchema.safeParse(body);

    if (!result.success) {
      return NextResponse.json(
        { code: "VALIDATION_ERROR", message: "Invalid field values", errors: result.error.issues },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({ code: "INTERNAL_ERROR", message: "Failed to update business" }, { status: 500 });
  }
});
 ...
 */
export const DELETE = withAuth(async (req, payload, context: { params: Promise<{ slug: string }> }) => {
  try {
    const { slug } = await context.params;
    ...
    return NextResponse.json({ code: "INTERNAL_ERROR", message: "Failed to delete business" }, { status: 500 });
  }
});
